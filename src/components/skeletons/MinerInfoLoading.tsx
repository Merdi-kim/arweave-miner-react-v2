const MinerInfoLoading = () => {
  return (
    <div className='py-10'>
      ...
      <div className='mt-10'>
        { Array(4).fill("").map((_, index) =>  <PartitionLoading key={index}/>
        )}
      </div>
    </div>
  )
...
const TotalRateLoading = ({title}:{title:string}) => {
    return (
        <div className='flex-none h-18 w-[140px] md:w-[180px] mx-1 md:mx-2 border border-gray-100 bg-white rounded-xl px-2 py-1 md:px-3 md:py-2'>
          <div className="text-gray-700 font-medium text-xs md:text-sm mb-4 md:mb-3">{title}</div>
        ...
        <div className="flex gap-2 md:gap-20 text-xs sm:text-base sm:min-w-[400px]">
            <div className="flex-1">
                ...
                <div className="flex flex-col gap-1">
                    <div className="flex items-center">Current : <div className="ml-2 h-3 md:h-5 w-20 bg-gray-200 animate-pulse"></div> </div>
                    <div className="flex items-center">Ideal : <div className="ml-2 h-3 md:h-5 w-20 bg-gray-200 animate-pulse"></div></div>
                    <div className="flex items-center">% of Ideal : <div className="ml-2 h-3 md:h-5 w-16 bg-gray-200 animate-pulse"></div></div>
                </div>
            </div>
            <div className="flex-1">
