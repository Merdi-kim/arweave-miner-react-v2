...
const Partition:FC<{metrics:PrometheusMetrics, performanceRates:Array<{[key: string]: string;}>}> = ({metrics, performanceRates}) => {
   const MODULE_MAX_SIZE = 3.6 * ONE_TERABYTE;
   const data_size = Number(Number(metrics.value || 0) / ONE_TERABYTE).toFixed(2)
   const data_size_percentage = ((Number(metrics.value || 0) / MODULE_MAX_SIZE) * 100).toFixed(2)
   ...
   const read_percentage = ideal_read_rate_num === 0 
  ? "0.00" 
  : ((read_rate_num / ideal_read_rate_num) * 100).toFixed(2);
   ...
   const hash_percentage = ideal_hash_rate_num === 0 
  ? "0.00" 
